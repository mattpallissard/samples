...
static inline bool gt(uint64_t i, uint64_t j) {
	return i > j;
}

static inline bool eq(uint64_t i, uint64_t j) {
	return i == j;
}
...
static inline void swap(uint64_t *i, uint64_t *j) {
	uint64_t k = *i;
	*i = *j;
	*j = k;
}
...
// the divide and conquor part of the exam
