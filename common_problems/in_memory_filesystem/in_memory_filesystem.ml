...
type erros  = Use_mkdir_for_dirs | Exists_dir | Exists_file
module FS = struct
  ...
  let write tree path contents = match contents with
    ...
    let rec aux root = function
      ...
      | h :: t ->
         ...
         match get path tree with
          | true -> aux path t
        |(match overwrite with false -> match t with
